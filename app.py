...
TAB_MARKET, TAB_STOCK, TAB_SCHOOL, TAB_GLOSSARY, TAB_HOW = (
    "🏠 Marktübersicht", "🔎 Aktie analysieren", "📚 Chart-Schule", "📖 Lexikon", "⚙️ So funktioniert's")
...
@st.cache_data(ttl=6 * 3600, show_spinner="Kennzahlen werden geladen …")
def get_fundamentals(source: str, ticker: str, price: float) -> Fundamentals:
    return synthetic_fundamentals(ticker, price) if source == DEMO else fetch_fundamentals(ticker)


@st.cache_data(ttl=6 * 3600, show_spinner="Kennzahlen für alle Aktien werden geladen (einmalig, ca. 10–30 s) …")
def get_fundamentals_many(source: str, items: tuple[tuple[str, float], ...]) -> dict[str, Fundamentals]:
    if source == DEMO:
        return {t: synthetic_fundamentals(t, p) for t, p in items}
    return fetch_many([t for t, _ in items])
...
with st.sidebar:
    ...
    show_funda = st.toggle("📊 Kennzahlen in der Übersicht", value=False,
                           help="Zeigt KGV, KBV und Verschuldungsgrad für jede Aktie der Liste. Das Laden dauert "
...
tabs = st.tabs([TAB_MARKET, TAB_STOCK, TAB_SCHOOL, TAB_GLOSSARY, TAB_HOW], key="nav", on_change="rerun")
...
with tabs[0]:
    ...
    funda_cols: list[str] = []
    if show_funda:
        funda = get_fundamentals_many(source, tuple((r.ticker, r.price) for r in recs))
        view["KGV"] = [funda[t].pe if t in funda else None for t in view["Ticker"]]
        view["KBV"] = [funda[t].pb if t in funda else None for t in view["Ticker"]]
        view["Debt/Equity"] = [funda[t].debt_to_equity if t in funda else None for t in view["Ticker"]]
        funda_cols = ["KGV", "KBV", "Debt/Equity"]
    ...
    st.dataframe(
        ...
        column_order=["Name", "Ticker", "Empfehlung", "Score", "Datum", "Kurs", *funda_cols, "Kursziel",
                      "Potenzial", "Stop-Loss", "Signalstärke", "Seit (Tage)", "Tendenz"],
        column_config={
            ...
            "KGV": st.column_config.NumberColumn(format="%.1f", help=tip("kgv")),
            "KBV": st.column_config.NumberColumn(format="%.2f", help=tip("kbv")),
            "Debt/Equity": st.column_config.NumberColumn("Verschuldung (D/E)", format="%.2f",
                                                         help=tip("verschuldungsgrad")),
        },
    )
    export = recommendations_table(recs).drop(columns=["Label"])
    if funda_cols:
        export = export.merge(view[["Ticker", *funda_cols]], on="Ticker", how="left")
        ...
        try:
            ...
            with sub[0]:
                ...
                if rec.side == BUY:
                    ...
                    m[1].metric("Stop-Loss", fmt_price(rec.stop_loss, rec.currency),
                                fmt_pct(rec.stop_loss / rec.price - 1), **CARD, help=tip("stop_loss"))
                ...
                how_to([
                    "**Blaue Linie** = Kurs (Schlusskurs je Handelstag), **orange** = 50-Tage-Linie, "
                    "**grün** = 200-Tage-Linie.",
                    "Liegt Blau über Grün, ist der langfristige Trend intakt. Kreuzt Orange die grüne Linie nach "
                    "oben, ist das ein **Golden Cross** (positiv), nach unten ein **Death Cross** (negativ).",
                    "**Grün gestrichelt** = Kursziel, **rot gestrichelt** = Stop-Loss, **grau gepunktet** = Kurse, ab "
                    "denen sich die Empfehlung ändern würde.",
                    "Die **senkrechte gepunktete Linie** markiert das Datum „Verkauf ab“ bzw. „Halten bis“.",
                    "Mit der Maus über den Chart fahren zeigt genaue Werte; Ziehen zoomt, Doppelklick setzt zurück.",
                    "Mehr dazu im Tab **📚 Chart-Schule**.",
                ])

            with sub[1]:
                funda = get_fundamentals(source, ticker, rec.price)
                metrics = rate(funda)
                html(ui.section("📊 Kennzahlen des Unternehmens",
                                f"{ui.term('Fundamentaldaten', 'fundamentalanalyse')} ergänzen die Chart-Analyse: "
                                "Wie teuer ist die Aktie, wie hoch verschuldet, wie profitabel?"))
                if not funda.available:
                    st.info("Für dieses Wertpapier liefert Yahoo Finance keine Kennzahlen (z. B. bei ETFs oder "
                            "Indizes) – oder die Verbindung ist gerade gestört.")
                else:
                    meta = " · ".join(x for x in (funda.sector, funda.industry) if x)
                    note = "🧪 Demo-Kennzahlen (simuliert)" if funda.is_demo else "Quelle: Yahoo Finance, letzte 12 Monate"
                    st.caption(f"{meta + ' · ' if meta else ''}{note}. Die Ampeln folgen allgemeinen Faustregeln – "
                               "was „teuer“ ist, hängt auch von der Branche ab.")
                    html(ui.fundamentals_summary(summarize(metrics)))
                    for group in GROUPS:
                        items = [x for x in metrics if x.group == group]
                        if items:
                            html(ui.section(group))
                            html(ui.metric_grid(items))

            with sub[2]:
                reading = read_chart(df)
                html(ui.section("🧭 Was zeigt der Chart gerade?",
                                "Die App liest den Chart wie ein Chartanalyst und zählt die Zeichen für steigende "
                                "und fallende Kurse."))
                ...
                with left:
                    ...
                    how_to([
                        "Die **nummerierten Punkte** gehören zu den Zeichen in der Liste: **grün** spricht für "
                        "steigende, **rot** für fallende Kurse.",
                        "**Grün gepunktet** = nächste Unterstützung (Boden), **rot gepunktet** = nächster Widerstand "
                        "(Decke).",
                        "Zeichen ohne Punkt (z. B. RSI oder Volumen) beziehen sich auf die letzten Tage.",
                    ])
                with right:
                    html(ui.reading_list(reading, LESSON_TITLES))
                st.warning("Kein Zeichen ist sicher. Je mehr Zeichen in dieselbe Richtung zeigen, desto "
                           "wahrscheinlicher – aber nie garantiert – setzt sich die Bewegung fort.", icon="⚠️")
                ...
                how_to([
                    ...
                    "Verkaufsdruck.",
                    "**RSI:** Linie im rötlichen Bereich über 70 = überkauft (Pause wahrscheinlicher), im grünlichen "
                    "Bereich unter 30 = überverkauft (Erholung möglich).",
                    "**MACD:** Kreuzt die blaue Linie die orange nach oben, nimmt der Schwung zu; nach unten lässt er "
                    "nach. Die Balken zeigen den Abstand.",
                ])
            ...
            with sub[3]:
                ...
                with right:
                    ...
                    how_to([
                        "Jeder Balken ist ein Faktor. **Nach rechts (blau)** spricht er für die Aktie, **nach links "
                        "(rot)** dagegen.",
                ...
                how_to([
                    "Die Linie ist der **Signal-Score**, die Gesamtnote der Aktie. Die farbigen Zonen entsprechen den "
                    "fünf Empfehlungen.",
                    "Wechselt die Linie in eine andere Zone, ändert sich die Empfehlung. Steigt sie, verbessert sich "
                    "das Bild.",
                ])
...
with tabs[2]:
    html(ui.section("📚 Chart-Schule",
                    "In 8 kurzen Lektionen lernen, wie man Charts liest – und woran man steigende oder fallende "
                    "Kurse erkennt."))
    st.info("Charts zeigen **Wahrscheinlichkeiten, keine Gewissheiten**. Kein einzelnes Zeichen ist sicher – "
            "achte darauf, ob mehrere Zeichen in dieselbe Richtung zeigen, und sichere dich mit einem Stop-Loss ab.",
            icon="💡")
    if st.session_state.get("lesson") not in LESSON_KEYS:
        st.session_state.lesson = LESSON_KEYS[0]

    def step_lesson(delta: int) -> None:
        st.session_state.lesson = LESSON_KEYS[(LESSON_KEYS.index(st.session_state.lesson) + delta) % len(LESSON_KEYS)]

    st.segmented_control("Lektion", LESSON_KEYS, key="lesson", required=True, label_visibility="collapsed", width="stretch",
                         format_func=lambda k: f"{LESSON_KEYS.index(k) + 1} · {SHORT[k]}")
    ...
    chart = charts.lesson_chart(lesson.key, palette())
    if chart is None:
    ...
    b1.button("← Vorherige Lektion", on_click=step_lesson, args=(-1,), width="stretch")
    b2.button("Nächste Lektion →", on_click=step_lesson, args=(1,), width="stretch", type="primary")

    html(ui.section("✅ Checkliste: Woran erkenne ich, wohin es geht?",
                    "Je mehr Punkte einer Spalte zutreffen, desto klarer das Bild. Der „🧭 Chart-Leser“ bei jeder "
                    "Aktie prüft diese Punkte automatisch."))
    html(ui.checklist())
